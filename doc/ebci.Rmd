...
i}=\frac{\mu_{2}}{\mu_{2}+\sigma_{i}^{2}}$ is the degree of shrinakge, and
...
second moment of the conditioanl bias of $\hat{\theta}_i$, scaled by the
...
knitr::kable(data.frame(cz=df$czname, unshrunk_estimate=df$theta25,
             ...
             digits=3)
```
Figure 6 in @akp20 presents the same information in graphical form.

Finally, let us compute some summary statistics as in Table 3 in @akp20. Average
length of the robust, parametric, and unshrunk CI:
...
mean(r$df$len_us)
```
Therefore, the efficiency of the parametric and unshrunk CI relative to the robust EBCI is given by
```{r}
