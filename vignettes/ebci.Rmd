...
These critical values are implemented in the package by the `cva` function that
takes the square root of $m_2$ as its first argument:
...
2. Estimates of $\sqrt{\mu_2}$ and $\kappa$, obtained using method of moments. We
   set `kappa=3`, so the kurtosis value is taken to be infinity:
   ```{r}
...
```
On the other hand, using the parametric EBCI yields CIs that midlly violate the
